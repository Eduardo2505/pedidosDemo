                                ...
                                if (isset($tiketEntradas)) {
                                    foreach ($tiketEntradas->result() as $rowx) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo strtoupper($rowx->useraceptado); ?></td>
                                            <td><?php echo strtoupper($rowx->estado); ?></td>
                                            <td>
                                               <?php 
                                               $variable=$rowx->urlArchivo;
                                               if(is_null($variable)){
                                                echo "Sin Guía";
                                            }else{

                                                ?>
                                                <a target="_blank" href="<?php echo $this->config->item('url_clusters'); ?><?php echo $rowx->urlArchivo; ?>" ><span class="icon-archive"></span> Descargar</a>
                                                <?php
                                        ...
                                        <td>
                                            ...
                                            <a href="#modal_default_3" data-toggle="modal" class="optenerID" alt="<?php echo $rowx->estado; ?>" title="<?php echo $rowx->idTiket; ?>" name="<?php echo str_pad($rowx->folio, 5, "0", STR_PAD_LEFT); ?>"><span class="icon-plus-sign"></span> Ver</a><br>
                                            <?php if ($acceso != 3) { ?>
                                            <a href="<?php echo site_url('') ?>pedidosadmin/cargarEvidencia?folio=<?php echo $rowx->folio; ?>&idTiket=<?php echo $rowx->idTiket; ?>" ><span class="icon-file-alt"></span> Cargar </a> 
                                            <?php } ?>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.optenerID').click(function() {
                    ...
                    if (estado != 'ACEPTADO') {
                        $('#botonbt').html('<a  class="btn btn-warning btn-clean" href="<?php echo site_url('') ?>pedidosadmin/imprimirRecupera?idTiket=' + user + '">ACEPTAR PEDIDO</a>');
                    }
                    $('#foliox').html('PEDIDO con el FOLIO ' + folio);
                    $('#boton').html('<a  class="btn btn-warning btn-clean" href="<?php echo site_url('') ?>pedidosadmin/reimprimir?idTiket=' + user + '">REIMPRIMIR</a>');
                    var dataString = 'idtiket=' + user;

                    $.ajax({
                        type: "GET",
                        url: "<?php echo site_url('') ?>pedidosadmin/verproductos",
                        data: dataString,
